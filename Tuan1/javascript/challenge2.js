...
const output = (firstPerson, secondPerson, testCaseNumber) => {
  ...
  if (firstPersonHigherBMI)
    console.log(
      `${firstPerson.name}'s BMI (${firstPersonBMI}) is higher than ${secondPerson.name}'s BMI (${secondPersonBMI})`
    );
  else
    console.log(
      `${secondPerson.name}'s BMI (${secondPersonBMI}) is higher than ${firstPerson.name}'s BMI (${firstPersonBMI})`
    );
};
